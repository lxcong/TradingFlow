<h1 align="center">TradingFlow</h1>

<p align="center">
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg?style=flat-square" alt="License: MIT" /></a>
  <img src="https://img.shields.io/badge/Claude%20Code-Dynamic%20Workflow-8A3FFC?style=flat-square" alt="Claude Code Dynamic Workflow" />
</p>

<p align="center">
A multi-agent equity-analysis <b>workflow for Claude Code</b> that mirrors the structure of a real
trading firm — reproducing its specialized roles and debate dynamics as a single Claude Code
...
The Export phase writes two artifacts to `outDir/` (default `tradingflow-reports/`), named
`<ticker>-<date>`:
...
This drops the script into `~/.claude/workflows/tradingflow.js`, where Claude Code auto-discovers it
as the `/tradingflow` command.

Or from a clone:

```bash
git clone https://github.com/lxcong/TradingFlow.git
cd TradingFlow
./install.sh                  # into ~/.claude/workflows/ (user scope)
./install.sh --project        # into ./.claude/workflows/ instead
```

**Prerequisites:** Claude Code **v2.1.154+**, a paid plan, and Dynamic workflows enabled in `/config`.
...
```
/tradingflow NVDA as of 2026-05-28
```

A bare `/tradingflow` runs with the defaults (NVDA, most recent trading day). The first run asks
...
```
Run the tradingflow workflow on 0700.HK as of 2026-05-28
```
...
```js
Workflow({
  scriptPath: "tradingflow.workflow.js",
...
![TradingFlow running in the Claude Code /workflows view: the Phases panel (Analysts → Research Debate → Research Verdict → Trader → Risk Debate → Portfolio Manager) beside the four parallel analyst agents](assets/run-progress.png)
...
| `outDir` | `tradingflow-reports` | Folder the report `.md` / `.html` are written to (object/`Workflow()` args only) |

> When run as the `/tradingflow` slash command, only `ticker` and `date` are read from the trailing
...
[MIT](LICENSE) © 2026 lxcong and contributors.
